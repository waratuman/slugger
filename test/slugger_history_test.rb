...
class SluggerHistoryTest < Minitest::Test
  ...
  test 'slug history added on update (before_save callback)' do
    ...
    assert_equal ['serenity', 'the-picture-of-dorian-gray'], Slugger::Slug.all.map(&:slug).sort
  end
  ...
  test 'slug history added on destroy' do
    ...
    movie.destroy
    assert_equal ['serenity', 'serenity', 'the-picture-of-dorian-gray'], Slugger::Slug.all.map(&:slug).sort
  end
